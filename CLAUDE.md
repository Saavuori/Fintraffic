...
- **Dependencies**: Dependabot (`.github/dependabot.yml`) watches all five pinned surfaces — `backend/` gomod, `frontend/` npm, GitHub Actions, the Dockerfile, and `deploy/` compose images — weekly, grouping minor/patch per ecosystem and splitting majors. `dependabot-changelog.yml` commits a generated entry (`scripts/dependabot-changelog.js`) onto each of its PRs; that text is factual only, so expand it by hand when a bump actually matters. Pushing to a Dependabot branch stops its auto-rebasing, so a predicted version can land stale if a sibling PR merges first — fix the heading rather than leaving it mismatched.
